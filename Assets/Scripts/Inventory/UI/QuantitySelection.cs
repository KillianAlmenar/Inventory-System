...
public class QuantitySelection : MonoBehaviour
{
    ...
    public void OkPressed()
    {
        ...
        foreach (Item testedItem in currentInventory.items)
        {
            if (quantity > 0)
            {
                if (testedItem == item && testedItem.type == Item.TYPE.CONSOMMABLE)
                {
                ...
                if (testedItem == item && testedItem.type == Item.TYPE.RESSOURCE)
                {
        ...
        foreach (Item testedItem in itemToDestroy)
        {
